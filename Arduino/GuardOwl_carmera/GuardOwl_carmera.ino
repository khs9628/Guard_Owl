...
const char *ssid = "LeeSH"; // Put your SSID here
const char *password = "29097595"; // Put your PASSWORD here

static IPAddress ip(192, 168, 43, 10); // static IP used for browser access: http://IPaddress
...
void handleNotFound() {
  ...
  else
  {
    ...
    {
      message += "<td><img src=\"http://"+ipStr+"/pics/" + String(i) + ".jpg\">" + i + ".jpg/></td>\n";
...
void loop() {
  ...
  if(lost<0)lost=0;
  sec++;
  if(sec % 60 == 0) capture = 0;
  if(val>impact||val<lost){
    if(capture != 1){
      handleSubmit();
      capture = 1;
    }
